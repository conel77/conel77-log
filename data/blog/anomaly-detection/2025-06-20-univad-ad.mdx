...
- **Grounded SAM**을 사용하여 텍스트 태그에 따라 **component 마스크들**을 생성
    ...
    → 이때 생성된 마스크 집합을 **Msam ∈ ℝᴹ×ᴴ×ᵂ**로 표현
...
이 클러스터 중심을 기반으로, 각 정상 이미지와 쿼리 이미지의 feature map 상 모든 위치 벡터가 어떤 클러스터에 속하는지 유사도(예: cosine similarity)에 따라 결정하고, 그 결과를 이용해 **클러스터 마스크(Mcluster ∈ ℝᴺ×ᴴ₁×ᵂ₁)** 를 생성한다. 이 마스크는 N개의 채널로 구성되며, 각 채널은 특정 클러스터에 속하는 위치들을 이진값(1 또는 0)으로 표시한다. 즉, 각 마스크는 “같은 의미를 공유하는 영역”을 하나의 컴포넌트로 표현한 것이라고 볼 수 있다. 
...
이러한 방식으로 배경 마스크를 제거한 후 남는 유효한 마스크 집합을 $M_{valid} ∈ ℝ^{N’ × H₁ × W₁}$로 정의하며, 이후 Grounded SAM의 마스크와 IoU 기반 매칭을 통해 최종적으로 통합된 component 마스크를 구축하게 된다.
...
이를 위해 UniVAD는 각 SAM 마스크에 대해 모든 클러스터 마스크와의 IoU를 계산하고, 그 중 가장 높은 IoU를 갖는 클러스터 마스크를 찾는다. 즉, SAM 마스크와 가장 많이 겹치는 클러스터 마스크가 해당 SAM 마스크의 대표 클러스터로 간주된다. 수식으로 표현하면, 각 SAM 마스크 M_{sam}^{(i)}에 대해 가장 유사한 클러스터 마스크 M_{valid}^{(j)}는 다음과 같이 할당된다:
$\text{Label}(M_{sam}^{(i)}) = \arg\max_j \text{IoU}(M_{sam}^{(i)}, M_{valid}^{(j)})$

이 과정을 통해 모든 SAM 마스크는 하나의 클러스터 인덱스에 대응되며, 이후 각 클러스터 인덱스 j에 대해 해당 클러스터에 할당된 SAM 마스크들을 모두 합집합(union)한다. 이렇게 해서 얻어진 마스크는 $M^{(j)} = \bigcup_{\text{Label}(M_{sam}^{(i)}) = j} M_{sam}^{(i)}$ 로 표현되며, 최종적으로 하나의 컴포넌트를 대표하는 통합된 마스크가 된다.
...
- 각 SAM 마스크 $M_i^{sam}$와 클러스터 마스크 $M_j^{valid}$ 간의 **IoU (교차-합 비율)** 계산
- 가장 높은 IoU를 갖는 클러스터 마스크에 SAM 마스크를 할당한다.
    
    ```
    Label(M_i^{sam}) = argmax_j IoU(M_i^{sam}, M_j^{valid})
    ```
...
가장 먼저, 쿼리 이미지와 정상(reference) 이미지를 사전학습된 이미지 인코더(예: ResNet, ViT 등)를 통해 처리하여 feature map을 추출합니다. 이 과정에서 쿼리 이미지에 대해서는 $F_q \in \mathbb{R}^{H_1 \times W_1 \times C}$, 그리고 K개의 정상 이미지에 대해서는 $F_n \in \mathbb{R}^{K \times H_1 \times W_1 \times C}$ 형태의 특징 맵이 생성됩니다.

이후에는 특징 맵을 일정한 크기로 보간(interpolation)하여, 패치 단위의 특징을 갖는 $P_q \in \mathbb{R}^{H_2 \times W_2 \times C}, P_n \in \mathbb{R}^{K \times H_2 \times W_2 \times C}$형태의 **patch-level feature**로 변환합니다. 이렇게 얻어진 패치들 간의 유사도를 바탕으로, 쿼리 이미지의 각 패치  $P_i^q$와 정상 이미지들의 모든 패치 P_n 사이의 cosine 거리 중 **최소값**을 취해 해당 패치의 이상 점수로 사용합니다. 이 방식은 다음과 같이 수식으로 표현됩니다:

$\text{Score}_{pm}(P_i^q) = \min(\text{distance}(P_i^q, P_n))$

이와 같은 전통적인 patch feature matching은 빠르고 직관적이지만 몇 가지 중요한 한계가 있습니다. 예를 들어, 이미지의 **배경 영역과 전경(관심 객체) 영역을 구분하지 못하기 때문에**, 배경에서의 패턴 변화에도 이상 점수가 높게 나오는 **false positive**가 발생할 수 있습니다. 

또한, 서로 다른 컴포넌트(예: 다른 부품) 사이에서 색상이나 질감이 비슷한 경우, 전혀 상관없는 부분끼리 잘못 매칭되는 문제가 생기기도 합니다.

이러한 문제를 해결하기 위해 UniVAD는 컴포넌트 기반(component-aware)의 보완 전략을 도입합니다. 앞서 3.2절에서 설명된 C3 모듈을 통해 얻은 **컴포넌트 마스크**를 활용하여, 이미지의 각 패치를 **컴포넌트 단위로 분리**합니다. 즉, 각 정상 이미지의 패치 특징 P_n에서, 특정 컴포넌트 마스크 M_i에 해당하는 위치의 패치들만 모아 하나의 집합 $P_n^i$ 를 구성합니다. 쿼리 이미지의 패치 P_q도 동일한 방식으로 컴포넌트별로 나누어 $P_q^i$를 생성합니다.

이제 쿼리 이미지의 각 컴포넌트 내 패치 $P_j^{qi}$는, 해당 컴포넌트에 해당하는 정상 패치 집합 $P_n^i$ 와만 비교됩니다. 그리고 이들 사이의 cosine 거리 중 최소값을 이상 점수로 사용하게 되며, 이는 다음 수식으로 표현됩니다:

$\text{Score}_{aware}(P_j^{qi}) = \min(\text{distance}(P_j^{qi}, P_n^i))$

이 방식은 **서로 다른 컴포넌트 간의 매칭을 방지**하고, **정확한 의미 단위 내에서의 비교**를 가능하게 하여 탐지 정확도를 크게 높입니다.

이와 함께, UniVAD는 멀티모달 특징을 활용하는 **이미지-텍스트 기반의 비교 방식**을 추가합니다. 이는 단순히 색상이나 질감만으로는 파악하기 어려운 개념적 이상을 감지하기 위해 도입된 것입니다. 먼저, "정상 상태"와 "이상 상태"를 설명하는 텍스트 문장을 준비하고, 이를 사전 학습된 텍스트 인코더(CLP, BERT 등)를 통해 임베딩합니다. 예를 들어:

- 정상을 나타내는 텍스트: `"a normal machine part"` → $T_n \in \mathbb{R}^C$
- 이상을 나타내는 텍스트: `"a broken or missing component"` → $T_a \in \mathbb{R}^C$

그 다음, 쿼리 이미지의 각 패치 특징 $P_j^{qi}$ 와 이 두 텍스트 임베딩 간의 **cosine similarity**를 구하고, softmax를 적용하여 각 패치가 정상이 더 가까운지, 이상이 더 가까운지를 나타내는 **확률적 점수**를 생성합니다:

$\text{Score}_{vl}(P_j^{qi}) = \text{softmax}(\text{sim}(P_j^{qi}, [T_n, T_a]))$

이렇게 하면 해당 패치가 **개념적으로 이상한지 여부**도 반영할 수 있게 되어, 의미 기반 탐지 정확도가 향상됩니다.

마지막으로, 위에서 설명한 세 가지 점수 —

(1) 기존 patch matching 점수 $\text{Score}_{pm},$

(2) 컴포넌트-aware 점수 $\text{Score}_{aware},$

(3) 이미지-텍스트 점수 $\text{Score}_{vl}$ —

를 가중합하여 최종적인 구조적 이상 점수 맵 $\text{Score}_{stru}$를 계산합니다:

$\text{Score}_{stru} = \alpha \cdot \text{Score}_{pm} + \beta \cdot \text{Score}_{aware} + \gamma \cdot \text{Score}_{vl}$

실험에서는 각 가중치 $\alpha, \beta, \gamma$ 를 동일하게 1/3으로 설정하여 평균을 사용했습니다. 이 최종 구조적 이상 점수는 이후의 GECM에서 생성된 논리적 이상 점수와 결합되어 UniVAD의 최종 anomaly map으로 이어지게 됩니다.
...
- 컴포넌트 마스크 $M_i \in ℝ^{H×W}$ 를 사용해
    ...
    $Pni={Pjn∣Mij=1}(4)P_n^i = \{ P_j^n \mid M_i^j = 1 \}$  4
...
- 각 patch의 anomaly score는 해당 컴포넌트 내 정상 패치와의 거리 최소값:
    ...
    $\text{Score}_{aware}(P_j^{qi}) = \min(\text{distance}(P_j^{qi}, P_n^i))$  5
...
- 각 쿼리 패치 특징 PjqiP_j^{qi}Pjqi에 대해 $T_n, T_a$ 와의 유사도:
    
    $\text{Score}_{vl}(P_j^{qi}) = \text{softmax}(\text{sim}(P_j^{qi}, [T_n, T_a]))$ 6
...
위의 세 가지 score를 결합해서 최종 structural anomaly map을 생성합니다:

$\text{Score}_{stru} = \alpha \cdot \text{Score}_{pm} + \beta \cdot \text{Score}_{aware} + \gamma \cdot \text{Score}_{vl}$  7
...
이 정보들을 결합하여, 쿼리 이미지에 대해  $Gq​∈ \mathbb{R}^{N_q \times C_g}$, 정상 이미지에 대해 $G_n \in \mathbb{R}^{N_n \times C_g}$ 형태의 **geometric feature vector**를 구성한다.

이후, 각 쿼리 컴포넌트 G_i^q에 대해 정상 컴포넌트들과의 거리 중 최소값을 **geometric anomaly score**로 다음과 같이 정의한다. 
...
$S_{ij} = \frac{S'_{ij}}{\sum_{k=1}^N S'_{ik}}, \quad S'_{ij} = \text{sim}(node_i, node_j)$ 9 
...
$\text{Score}_{deep}(E_i^q) = \min(\text{distance}(E_i^q, E_n))$ 10
...
$\text{Score}_{logic} = φ \cdot \text{Score}_{deep} + ψ \cdot \text{Score}_{geo}$ 12
...
$\text{Score}_{final} = δ \cdot \text{Score}_{stru} + η \cdot \text{Score}_{logic}$  13
...
1. **3.2 C3 모듈**: 컴포넌트 단위로 이미지를 분할
2. **3.3 CAPM 모듈**: 각 패치의 이상 정도를 세 가지 방식으로 평가 (기존 매칭 + 컴포넌트 제한 + 텍스트)
3. **3.4 GECM 모듈**: 컴포넌트 간 관계와 속성으로 고차원적 논리 이상 탐지
