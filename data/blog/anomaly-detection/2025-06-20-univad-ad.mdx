...
이러한 문제를 해결하기 위해 UniVAD는 컴포넌트 기반(component-aware)의 보완 전략을 도입합니다. 앞서 3.2절에서 설명된 C3 모듈을 통해 얻은 **컴포넌트 마스크**를 활용하여, 이미지의 각 패치를 **컴포넌트 단위로 분리**합니다. 즉, 각 정상 이미지의 패치 특징 $P_n$에서, 특정 컴포넌트 마스크 $M_i$에 해당하는 위치의 패치들만 모아 하나의 집합 $P_n^i$ 를 구성합니다. 쿼리 이미지의 패치 $P_q$도 동일한 방식으로 컴포넌트별로 나누어 $P_q^i$를 생성합니다.
